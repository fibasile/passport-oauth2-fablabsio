    ...
    function loaded(err, ok, state) {
      ...
      self._oauth2.getOAuthAccessToken(code, params,
        function(err, accessToken, refreshToken, params) {
          console.log('got oauth access token' + accessToken);
          // console.log('got oauth access token' + refreshToken);
          console.log(err)
...
OAuth2Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  console.log('https://api.fablabs.io/v0/me.json' + accessToken)
  this._oauth2.get('https://api.fablabs.io/v0/me.json',accessToken, function(err,data,response){
    if (err){
      return done(null, {});
    }
    ...
    var profile = {};
    if (data.user){
      profile.id = data.user.username || '';
      profile.fablabs_id = data.user.id || '';
      profile.username = data.user.username || '';
      profile.email = data.user.email || '';
      profile.avatar = data.user.avatar_url || '';
      profile.first_name = data.user.first_name || '';
      profile.last_name = data.user.last_name || '';
      profile.labs = data.user.employees || [];
    }
